...
export function parse(options: any): VerifyCmdOptions {
  ...
  const { verifyMeaningSeparatorUsed, verifyMeaningSeparatorNotUsed, verifyTagsUsed } = options;
  ...
  const arrOptions = [verifyMeaningSeparatorUsed, verifyMeaningSeparatorNotUsed, verifyTagsUsed];
  if (arrOptions.filter((v) => v === undefined).length < 2) {
